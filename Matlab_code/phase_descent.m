...
function [v_mean,dv,q,n,dh, alpha,CD0,alt_f, beta_phase] = phase_descent(alt_i, alt_f, descent_rate, speed, mach_i, mach_f,k1)
    ...
    if isempty(alt_f)
        ...
        if isempty(mach_f) & isempty(mach_i) 
            ...
            if isempty(speed)
                ...
                iter_max = 100;
                k = 0;
                while abs(v_air_f - v_air_f_deduced) > 1 & k < iter_max
                    v_f = convert_KEAS2KTAS(speed, alt_f);
                    v_air_f_deduced = v_f/mach_f;
                    [T_f,P_f, rho_f, v_air_f, delta_f, theta_f, sigma_f, alt_f] = inverse_air_condition(mach_f, v_f);
                    k=k+1;
                end
                k
                alt_f
